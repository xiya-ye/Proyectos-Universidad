...
namespace render {
  ...
  hit_opt scene::intersect(ray const & r) const noexcept {
    hit_opt best{};
    ...
    for (object const * obj : objects_) {
      if (obj == nullptr) {
        continue;
      }
      hit_opt const h = obj->intersect(r, scene::kLambdaMin, t_best);
      if (h) {
  ...
  color scene::trace(ray const & r, int depth) const noexcept {
    // Caso base: sin presupuesto de rebotes → sin contribución directa (negro).
    if (depth <= 0) {
      return color{0.0, 0.0, 0.0};
    }

    hit_opt const h = intersect(r);
    if (!h) {
    ...
    auto const scatter = h->mat->scatter(r, *h, material_rng_);
    if (!scatter) {
      // Absorción total (no hay rayo saliente).
      return color{0.0, 0.0, 0.0};
    }

    ray const & r_scattered = scatter->first;   // Or = I (lo fija scatter)
    color const & atten     = scatter->second;  // atenuación por material
