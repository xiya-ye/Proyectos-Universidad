...
namespace render {
  ...
  namespace {  // ----------------- helpers internos, no exportados -----------------
    ...
    inline std::vector<std::string> split_ws(std::string const & v) {
    ...
    [[noreturn]] inline void throw_unknown_key(std::string const & raw_key_no_colon) {
      throw std::runtime_error("Error: Unknown configuration key: [" + raw_key_no_colon + ":]");
    }

    [[noreturn]] inline void throw_invalid_value(std::string const & key_with_colon,
                                                 std::string const & whole_line) {
      std::string msg = "Error: Invalid value for key: [" + key_with_colon + "]\n";
      msg += "Line: \"" + whole_line + "\"";
      throw std::runtime_error(msg);
    }

    [[noreturn]] inline void throw_extra_data(std::string const & key_with_colon,
                                              std::string const & extra) {
      std::string msg =
          "Error: Extra data after configuration value for key: [" + key_with_colon + "]\n";
      msg += "Extra: \"" + extra + "\"";
      throw std::runtime_error(msg);
    }
    ...
    inline void ensure_no_extra(std::vector<std::string> const & t, std::size_t expected,
                                ParseCtx const & ctx) {
      if (t.size() > expected) {
    ...
    inline std::uint64_t parse_u64(std::string const & s) {
      std::size_t idx        = 0;
      ...
      try {
        ...
        throw std::invalid_argument("bad u64");
      }
      return static_cast<std::uint64_t>(tmp);
    }

    inline double parse_double(std::string const & s) {
      std::size_t idx = 0;
      double v        = 0.0;
      try {
        v = std::stod(s, &idx);
    ...
    inline vector parse_vec3(std::vector<std::string> const & t, ParseCtx const & ctx) {
    ...
    inline color parse_color3(std::vector<std::string> const & t, ParseCtx const & ctx) {
      ensure_no_extra(t, 3, ctx);
      color c{};
      try {
      ...
      bool const out = (c.r < 0.0 or c.r > 1.0 or c.g < 0.0 or c.g > 1.0 or c.b < 0.0 or c.b > 1.0);
      if (out) {
        throw_invalid_value(ctx.key, ctx.line);
      }
      return c;
    ...
    inline KeyType classify_key(std::string const & k) {
      static std::unordered_map<std::string, KeyType> const map{
    ...
    inline void handle_aspect_ratio(render_config & cfg, std::vector<std::string> const & t,
                                    ParseCtx const & ctx) {
    ...
    inline void handle_image_width(render_config & cfg, std::vector<std::string> const & t,
                                   ParseCtx const & ctx) {
    ...
    inline void handle_gamma(render_config & cfg, std::vector<std::string> const & t,
                             ParseCtx const & ctx) {
      ensure_no_extra(t, 1, ctx);
      double g = 0.0;
      try {
    ...
    inline void handle_fov(render_config & cfg, std::vector<std::string> const & t,
                           ParseCtx const & ctx) {
      ensure_no_extra(t, 1, ctx);
      double fov = 0.0;
      try {
        fov = parse_double(t[0]);
      } catch (...) {
        throw_invalid_value(ctx.key, ctx.line);
      }
    ...
    inline void handle_spp(render_config & cfg, std::vector<std::string> const & t,
                           ParseCtx const & ctx) {
      ensure_no_extra(t, 1, ctx);
      int spp = 0;
      try {
    ...
    inline void handle_max_depth(render_config & cfg, std::vector<std::string> const & t,
                                 ParseCtx const & ctx) {
      ensure_no_extra(t, 1, ctx);
      int md = 0;
      try {
    ...
    inline void handle_seed(render_config & cfg, std::vector<std::string> const & t,
                            ParseCtx const & ctx, bool material) {
      ensure_no_extra(t, 1, ctx);
      std::uint64_t s = 0ULL;
      try {
        s = parse_u64(t[0]);
      ...
      if (material) {
        cfg.material_rng_seed = s;
      } else {
        cfg.ray_rng_seed = s;
      }
    }

    inline void handle_background(render_config & cfg, std::vector<std::string> const & t,
                                  ParseCtx const & ctx, bool dark) {
      color const c = parse_color3(t, ctx);
      if (dark) {
        cfg.background_dark = c;
      } else {
        cfg.background_light = c;
      }
    }

    inline void handle_camera_vec(render_config & cfg, std::vector<std::string> const & t,
                                  ParseCtx const & ctx, int which) {
      vector const v = parse_vec3(t, ctx);
      if (which == 0) {
    ...
    inline void apply_kv(render_config & cfg, std::string const & key_with_colon,
                         std::string const & raw_value, std::string const & whole_line) {
      ParseCtx const ctx{key_with_colon, whole_line};
      auto const tokens = split_ws(raw_value);
      switch (classify_key(key_with_colon)) {
        ...
        default:                   throw_unknown_key(key_with_colon.substr(0, key_with_colon.size() - 1));
      }
    }

    // Parsing de una línea completa “clave: valor...”, con recorte y checks.
    inline void parse_line(render_config & cfg, std::string const & original, std::string line) {
      trim_inplace(line);
      if (line.empty()) {
        return;
      }  // líneas en blanco: ignorar
      ...
      std::string const key_with_colon = key + ":";
      if (value.empty()) {
    ...
    inline void validate_final(render_config const & cfg) {
  ...
  render_config render_config::parse(std::istream & is) {
    ...
    while (std::getline(is, line)) {
      std::string const & original = line;  // conserva la línea para errores
...
namespace render {  // ctor que carga desde ruta (I/O y delega en parse)
  ...
  render_config::render_config(std::string const & path) {
