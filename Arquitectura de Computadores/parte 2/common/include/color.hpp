...
namespace render {
  ...
  struct color final {
    ...
    [[nodiscard]] color apply_gamma(double gamma) const noexcept {
      ...
      return {std::pow(c.r, inv_gamma), std::pow(c.g, inv_gamma), std::pow(c.b, inv_gamma)};
    }

    // --- Conversión a 8-bit por canal ---------------------------------------
    // Devuelve (R,G,B) en [0,255] TRUNCANDO tras escalar. Útil para PPM P3.
    [[nodiscard]] constexpr std::tuple<std::uint8_t, std::uint8_t, std::uint8_t> to_rgb255()
        const noexcept {
      color const c = clamp();
      return {static_cast<std::uint8_t>(c.r * 255.0), static_cast<std::uint8_t>(c.g * 255.0),
  ...
  [[nodiscard]] constexpr color operator*(double s, color const & c) noexcept {
