...
namespace render::ppm {
  ...
  inline void write_pixels_ascii(std::ostream & os, image_size sz, Getter const & get,
                                 writer_opts opts = {}) {
    double const inv_gamma = (opts.gamma > 0.0) ? (1.0 / opts.gamma) : (1.0 / 2.2);
    for (int y = 0; y < sz.h; ++y) {    // filas de arriba a abajo
      for (int x = 0; x < sz.w; ++x) {  // columnas de izquierda a derecha
        color const c = get(x, y);
        int const ir  = to_u8(c.r, inv_gamma);
        int const ig  = to_u8(c.g, inv_gamma);
        int const ib  = to_u8(c.b, inv_gamma);
  ...
  inline void write(std::ostream & os, Image const & img, writer_opts opts = {}) {
    image_size const sz{img.width(), img.height()};
    write_header(os, sz);
    write_pixels_ascii(
        os, sz,
  ...
  template <class Getter>
  inline void write(std::ostream & os, image_size sz, Getter const & get, writer_opts opts = {}) {
    write_header(os, sz);
