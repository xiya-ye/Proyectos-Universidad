...
namespace render {
  ...
  struct camera_optics final {
    ...
    constexpr camera_optics(double fov, double asp) noexcept : fov_deg(fov), aspect(asp) { }

    // Si w,h > 0, la aspect efectiva es w/h
    constexpr camera_optics(double fov, int w, int h) noexcept
        : fov_deg(fov),
          aspect((h > 0) ? static_cast<double>(w) / static_cast<double>(h) : (16.0 / 9.0)),
          width(w), height(h) { }
  };
  ...
  public:
    ...
    camera(camera_pose const & pose, camera_optics const & opt) noexcept
      ...
      vector const dx = ph_ / static_cast<double>(std::max(1, img_w_));
      vector const dy = pv_ / static_cast<double>(std::max(1, img_h_));
    ...
    [[nodiscard]] ray primary_ray(double u, double v) const noexcept {
      vector const q = origin_window_ + ph_ * u + pv_ * v;
    ...
    [[nodiscard]] ray primary_ray_from_pixel(int f, int c, double dx, double dy) const noexcept {
      double const u = (static_cast<double>(c) + dx) / static_cast<double>(img_w_);
      double const v = (static_cast<double>(f) + dy) / static_cast<double>(img_h_);
