...
struct vector {
    ...
    [[nodiscard]] constexpr vector operator+(const vector& r) const noexcept { return {x + r.x, y + r.y, z + r.z}; }
    [[nodiscard]] constexpr vector operator-(const vector& r) const noexcept { return {x - r.x, y - r.y, z - r.z}; }
    [[nodiscard]] constexpr vector operator*(double s) const noexcept { return {x * s, y * s, z * s}; }
    [[nodiscard]] constexpr vector operator/(double s) const noexcept { return {x / s, y / s, z / s}; }

    // Operaciones en el propio objeto (más eficientes en bucles).
    constexpr vector& operator+=(const vector& r) noexcept { x += r.x; y += r.y; z += r.z; return *this; }
    constexpr vector& operator-=(const vector& r) noexcept { x -= r.x; y -= r.y; z -= r.z; return *this; }
    constexpr vector& operator*=(double s) noexcept { x *= s; y *= s; z *= s; return *this; }
    constexpr vector& operator/=(double s) noexcept { x /= s; y /= s; z /= s; return *this; }

    // --- Producto escalar y vectorial ----------------------------------------
    // Escalar (dot): mide proyección y sirve para ángulos/energía.
    [[nodiscard]] constexpr double dot(const vector& r) const noexcept {
        return x * r.x + y * r.y + z * r.z;
    }

    // Vectorial (cross): normal al plano (regla de la mano derecha).
    [[nodiscard]] constexpr vector cross(const vector& r) const noexcept {
        return { y * r.z - z * r.y,
                 z * r.x - x * r.z,
                 x * r.y - y * r.x };
    }
...
[[nodiscard]] constexpr vector operator*(double s, const vector& v) noexcept {
