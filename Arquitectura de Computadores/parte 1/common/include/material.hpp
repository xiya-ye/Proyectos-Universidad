...
public:
    ...
    explicit material(std::string name, color albedo) noexcept
      : name_(std::move(name)) 
      , albedo_{ std::clamp(albedo.r, 0.0, 1.0)
               , std::clamp(albedo.g, 0.0, 1.0)
    ...
    explicit material(std::string name, double ior) noexcept
      : name_(std::move(name))
      , type_(MaterialType::Refractive)
      , albedo_{1.0, 1.0, 1.0}
      , ior_(ior) {}

    // Dispersión: devuelve (rayo_saliente, atenuación) o nullopt (absorción).
    [[nodiscard]] std::optional<std::pair<ray, color>>
    scatter(const ray& in, const Hit& h, sampler& rng) const noexcept;
