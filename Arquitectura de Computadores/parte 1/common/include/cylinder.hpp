...
public:
    ...
    cylinder(const vector& c, double r, const vector& axis, const material* m) noexcept;
...
private:
    ...
    [[nodiscard]] double height_at(const vector& p) const noexcept;
    ...
    [[nodiscard]] hit_opt make_hit(const ray& r,
                                   double t,
                                   const vector& p,
                                   const vector& outward) const noexcept;
    ...
    const material* mat_ptr_{nullptr}; // material asociado (no propietario)
};
