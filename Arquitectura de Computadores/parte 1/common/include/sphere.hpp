...
public:
    ...
    sphere(const vector& c, double r, const material* m) noexcept
    ...
    [[nodiscard]] const material* material_ptr() const noexcept { return mat_ptr_; }

private:
    vector center_{0.0, 0.0, 0.0};          // C
    double radius_{1.0};                    // r > 0
    const material* mat_ptr_{nullptr};      // material asociado (no se posee)
};
