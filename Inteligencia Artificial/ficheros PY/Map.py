...
class Map:
    ...
    def compute_detection_map(self) -> np.array:
        ...
        # Inicializamos un mapa de detección con ceros
        detection_map = np.zeros(shape=(self.height, self.width), dtype=np.float32)

        # Coordenadas de cada celda
        latitudes = np.linspace(start=self.boundaries.min_lat, stop=self.boundaries.max_lat, num=self.height)
        longitudes = np.linspace(start=self.boundaries.min_lon, stop=self.boundaries.max_lon, num=self.width)
        ...
        for i in tqdm(range(self.height)):
            for j in range(self.width):
                ...
                # Inicializamos el mapa de detección con ceros
                detection_map[i][j] = 0.0

                # Loop para cada radar en el mapa
                for radar in self.radars:
                    # Computamos el nivel de detección del radar en la celda actual
                    detection_map[i][j] += radar.compute_detection_level(latitude=lat, longitude=lon)

        # Escalar la matriz usando la técnica MinMax
