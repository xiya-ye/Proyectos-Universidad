...
def main() -> None:
    ...
    # Después de crear el mapa y antes de path_finding:
    print(f"Dimensiones del detection_map: {detection_map.shape}")
    print(f"Dimensiones esperadas (W,H): ({M.width}, {M.height})")

    # Verifica que coincidan
    assert detection_map.shape == (
        M.height, M.width), "¡Las dimensiones no coinciden!"
