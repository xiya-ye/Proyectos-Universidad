    ...
    def sprite(self):
        ...
        if self.enemigo_enfadado == False:
            if self.enemigo_rebelde == False:
                if self.direccion == 1:
                    return constantes.MOSCA_DCH
                else:
                    return constantes.MOSCA_IZQ
            else:
                return constantes.MOSCA_REBELDE
        else:
            if self.direccion == 1:
                return constantes.MOSCA_ENFADADA_DCH
            else:
                return constantes.MOSCA_ENFADADA_IZQ

    def mover(self):
        """Con este método hacemos que la mosca se mueva.
        Cuando llega a los extremos aparece por el otro lado.
        Además, introducimos un salto a la mosca."""
        if self.direccion == 1:
            if self.x < 200:
...
# Definimos el método caer que controla que las moscas caigan cuando no
